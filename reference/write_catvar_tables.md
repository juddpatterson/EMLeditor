...
`write_catvar_tables` is a wrapper around get_catvar_tables(). It takes
an EML object and writes one categorical variable table for each data
table in the EML file. It writes the attribute tables as .txt files in
the working directory or a specified path. This function is useful for
