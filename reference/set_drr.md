...
- drr_ref_id:
  ...
  a 7-digit string that is the DataStore Reference ID for the DRR
...
- org_name:
  ...
  String. Defaults to NPS. If the organization publishing the DRR is
  *not* NPS, set org_name to your publishing organization's name.
...
adds uses the DataStore Reference ID for an associate DRR to the as a
properly formatted DOI (prefaced with "DRR: ") to the element. Creates
and populates required children elements for usageCitation including the
DRR title, creator organization name, and report number. Note the
organization name (org_name) defaults to NPS. If you do NOT want the
organization name for the DRR to be NPS, set org_name="Your Favorite
Organization" *and* set NPS=FALSE. Also sets the id flag for this
usageCitation to "associatedDRR".
